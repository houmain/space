import {
	MessageGameJoined, MessagePlayerJoined, ServerMessageType, MessageSquadronSent, MessageSquadronAttacks,
...
import { SendSquadron } from '../clientMessages';
...
export class MockMessageBuilder {
	...
	*//*
	public static createAvailableGameSessions() {
		...
		return {
			event: ServerMessageType.AVAILABLE_SESSIONS,
			sessions: [
				{
					name: 'Testsession',
					numPlayers: 1,
	...
	*/
	/*	public static createMessagePlayerReady(msg: PlayerReadyMessage): MessagePlayerReady {
		...
		public static createMessageStartGame(): MessageStartGame {
			let joinMsg: string;
			let addFighters = true;

			if (addFighters) {
				let numPlayerFighters = 25;
				joinMsg = '{"event":"startGame",' +
